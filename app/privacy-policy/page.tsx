...
export const metadata: Metadata = {
  title: "Privacy Policy - British Elite Betting Sites",
  description:
    "Read the Privacy Policy for British Elite Betting Sites (britishelitebettingsites.com). How we collect, use, and protect your data under UK GDPR.",
  alternates: { canonical: "https://britishelitebettingsites.com/privacy-policy" },
}
...
const sections: { id: string; title: string; content: React.ReactNode }[] = [
  {
    ...
    content: (
      <>
        <p>
          British Elite Betting Sites (britishelitebettingsites.com) is committed to protecting the privacy and personal data of every visitor. This Privacy Policy explains how we collect, use, store, and protect your information in compliance with the UK General Data Protection Regulation (UK GDPR) and the Data Protection Act 2018.
        </p>
        <p className="mt-3">
          By accessing britishelitebettingsites.com you acknowledge the practices described in this policy. This document applies only to data collected through this website. Linked third-party sites — including bookmaker operators — maintain their own privacy policies, which we encourage you to review independently.
        </p>
      </>
    ),
  },
        ...
        <ul className="space-y-2">
          {[
            ...
            <li key={item} className="flex items-start gap-2.5 text-sm">
              <span className="w-1.5 h-1.5 bg-primary flex-shrink-0 mt-1.5" aria-hidden="true" />
              {item}
            </li>
          ))}
        </ul>
  ...
  },
  {
    ...
    content: (
      <p className="text-sm">
        This Privacy Policy may be revised periodically. The &ldquo;Last updated&rdquo; date at the head of this page reflects the most recent revision. Continued use of britishelitebettingsites.com following any changes constitutes acceptance of the updated policy.
      </p>
    ),
  },
]
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-navy px-4 py-10 border-b border-white/10">
        <div className="container mx-auto max-w-5xl">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="font-heading font-extrabold text-2xl md:text-4xl text-white uppercase tracking-tight">Privacy Policy</h1>
          </div>
      ...
      <div className="container mx-auto max-w-5xl px-4 py-10 flex flex-col lg:flex-row gap-8">
        ...
        <aside className="hidden lg:block w-56 flex-shrink-0">
          <nav
            className="sticky top-6 bg-card border border-border p-4"
            ...
            <ul className="space-y-1">
              {sections.map((s) => (
                <li key={s.id}>
                  <a
                    href={`#pp-${s.id}`}
                    className="block text-xs text-muted-foreground hover:text-primary transition-colors py-0.5 font-sans"
                  >
                    {s.title}
                  </a>
                </li>
        ...
        >
          {sections.map((s) => (
            ...
            >
              <div className="border-b border-border px-6 py-4 border-l-4 border-l-primary">
                <h2 id={`pp-h-${s.id}`} className="font-heading font-bold text-base text-foreground uppercase tracking-tight">
                  {s.title}
                </h2>
              </div>
              <div className="px-6 py-5 text-muted-foreground leading-relaxed font-sans">
                {s.content}
              </div>
            </section>
          ))}
          ...
          <div className="bg-surface border border-border px-6 py-4">
            <p className="text-sm text-muted-foreground font-sans">
              ...
              <a
                href="mailto:privacy@britishelitebettingsites.com"
                className="text-primary hover:underline underline-offset-2 font-medium"
              >
                privacy@britishelitebettingsites.com
              </a>
            </p>
          </div>
        </main>
